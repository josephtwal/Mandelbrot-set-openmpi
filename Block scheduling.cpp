...
int main(int argc, char **argv)
{
    ...
    { //Master
        ...
        for (int j = 1; j < nproc; ++j)
        {
            ...
            startval = imgX * (id_from - 1) / (nproc - 1);
            endval = imgX * id_from / (nproc - 1);

            for(int i=startval;i<endval; ++i)
                MPI_Recv(&img_array[startval][0], (endval - startval) * imgY, MPI_INT, id_from,
    ...
    { //Slave
        ...
        startval = imgX * (id - 1) / (nproc - 1);
        endval = imgX * id / (nproc - 1);

        cx = cx + resX * startval; //jump to the right cx
        ...
        MPI_Send(&img_array[startval][0], (endval - startval) * imgY, MPI_INT, 0, 2, MPI_COMM_WORLD);
    }
